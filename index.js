...
// --- एक्शन्स ---
bot.start((ctx) => ctx.reply(`👑 Welcome Agrimm Official 👑\n👇 आज का गेम देखने के लिए बटन दबाएं:`, mainKB));

// Simple Reply Mode (Edits avoid kiye hain taaki error na aaye)
bot.action('HOME', (ctx) => { ctx.reply(`👑 Main Menu 👑`, mainKB); });
bot.action('F1', (ctx) => { ctx.reply(`📂 VIP Folder 1`, f1KB); });
bot.action('F2', (ctx) => { ctx.reply(`📂 VIP Folder 2`, f2KB); });

bot.action('G_SRI', (ctx) => { ctx.reply(marketData.SRIDEVI); });
bot.action('G_TIME', (ctx) => { ctx.reply(marketData.TIME); });
bot.action('G_MIL', (ctx) => { ctx.reply(marketData.MILAN); });
bot.action('G_KAL', (ctx) => { ctx.reply(marketData.KALYAN); });
bot.action('CH', (ctx) => { ctx.reply(marketData.CHART); });
